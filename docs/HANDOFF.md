...
## 現在地（最終更新: 2026-06-04）
- **App Store 3回目の提出済み（v1.0.0）。審査結果待ち。**
- 2回目リジェクト(Guideline 2.1 Information Needed): レビュアーが購入フローを試せない=テスト用アカウント未提供が原因。コードのバグではない。→ 対応: **クリーンな(Pro未購入)Sandboxテスター `625.somq2525+sbx2@gmail.com` を新規作成し、ASC「App Review Information」のNotes欄にアカウント情報+購入手順を記載**して再提出。新ビルド不要だった。
  - 注意: `+sbx1` は既にPro購入済み(TestFlight課金テストで使用)のためレビュー用には渡せない。レビュー用sbx2では購入テストしないこと(クリーンを維持)。
